...
def _get_all_settings():
    settings = {}
    try:
        from spotify_dl.settings import get_settings as spotify_dl_settings
        settings["spotify_dl"] = spotify_dl_settings()
    except ImportError:
        pass
    try:
...
@router.post("/settings/spotify-dl")
async def update_spotify_dl_settings(
    request: Request,
    max_workers_per_job: int = Form(...),
    max_workers_global: int = Form(...),
):
    _require_admin(request)
    from spotify_dl.settings import update_settings
    update_settings(
        max_workers_per_job=max(1, min(20, max_workers_per_job)),
        max_workers_global=max(1, min(100, max_workers_global)),
    )
    return templates.TemplateResponse(
        request, "admin/index.html", _admin_context(request, saved="spotify_dl"),
    )
