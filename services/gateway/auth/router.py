...
from auth.session import clear_session, get_current_user, set_session_user
...
def _user_hashed_password(user: User) -> str | None:
    return cast(str | None, getattr(user, "hashed_pw"))


def _user_auth_provider(user: User) -> str | None:
    return cast(str | None, getattr(user, "auth_provider"))


def _user_is_permanent(user: User) -> bool:
    return cast(bool, getattr(user, "is_permanent"))
...
async def login_submit(
    ...
    if user is not None:
        stored_hash = _user_hashed_password(user)
        if stored_hash and verify_password(password_hash, stored_hash):
        # Clear old session data first to prevent session fixation
            request.session.clear()
            set_session_user(request, _user_id_value(user))
...
def _cleanup_user_jobs(user_id: str) -> None:
    ...
    try:
        from spotify_dl.job_runner import cleanup_jobs_for_user as g2r_cleanup
        g2r_cleanup(user_id)
    except ImportError:
        pass
    try:
...
async def change_password(
    ...
    if (_user_auth_provider(user) or "") != "local":
        return templates.TemplateResponse(
            request,
            "auth/account.html",
    ...
    stored_hash = _user_hashed_password(user)
    if not stored_hash or not verify_password(current_hash, stored_hash):
        return templates.TemplateResponse(
            request,
            "auth/account.html",
    ...
    with get_db() as db:
        db_user = db.get(User, _user_id_value(user))
        if db_user is None:
            clear_session(request)
            return RedirectResponse("/login", 302)
        setattr(db_user, "hashed_pw", hash_password(new_hash))
        db.commit()
...
async def admin_create_user(
    ...
    if not username:
        from admin import _admin_context, _get_all_settings
        return templates.TemplateResponse(
            request,
            "admin/index.html",
